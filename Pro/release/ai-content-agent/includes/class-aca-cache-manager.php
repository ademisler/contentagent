...
class ACA_Cache_Manager {
    ...
    private static $cache_clear_triggers = array(
        ...
        'aca_pixabay_api_key',
        'aca_auto_publish',
        'aca_full_auto_daily_post_count',
        'aca_full_auto_publish_frequency',
        'aca_semi_auto_idea_frequency'
    );
    ...
    public static function clear_all_caches($reason = 'Manual', $full_flush = false) {
        ...
        try {
            ...
            $lock_key = 'aca_cache_clear_lock';
            $lock_value = get_transient($lock_key);
            
            if ($lock_value) {
                aca_debug_log('Cache clear already in progress, skipping');
                return array(
                    'success' => false,
            ...
            if (defined('AS_RUNNER_TICK') && AS_RUNNER_TICK) {
                ...
                aca_debug_log('Cache clear skipped - Action Scheduler is currently running');
                return array(
                    'success' => false,
                    'message' => __( 'Cache clear skipped while automation tasks are running. Please try again in a moment.', 'ai-content-agent' )
                );
            }
            ...
            update_option('aca_assets_version', time());
            ...
            update_option('aca_last_cache_clear', current_time('mysql'));
            
            // Log the cache clear
            aca_debug_log("Cache cleared - Reason: {$reason}, Items: " . implode(', ', $cleared_items));
            
            // Set transient for success message
            set_transient('aca_cache_cleared', array(
                'success' => true,
            ...
            delete_transient('aca_cache_clear_lock');
            
            aca_debug_log('Cache clear failed: ' . $e->getMessage());
    ...
    public static function auto_clear_cache($old_value, $new_value, $option) {
        ...
        // Set notice for user
        set_transient('aca_cache_auto_cleared', array(
            'option' => $option,
            'time' => current_time('mysql')
        ), 30);
    }
    ...
    public static function show_cache_notices() {
        ...
        $auto_cleared = get_transient('aca_cache_auto_cleared');
        if ($auto_cleared) {
            delete_transient('aca_cache_auto_cleared');
            ?>
        ...
        $cache_cleared = get_transient('aca_cache_cleared');
        if ($cache_cleared && $cache_cleared['success']) {
            delete_transient('aca_cache_cleared');
            ?>
    ...
    public static function get_cache_status() {
        ...
        // Get last clear time
        $last_clear = get_option('aca_last_cache_clear', 'Never');
        
        return array(
            'transient_count' => $transient_count,
            'object_cache_enabled' => $object_cache_enabled,
            'last_clear' => $last_clear,
            'assets_version' => get_option('aca_assets_version', '1.0')
        );
    }
}

// Initialize the cache manager
add_action('init', array('ACA_Cache_Manager', 'init'));