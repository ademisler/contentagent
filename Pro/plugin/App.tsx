...
import { SimpleAutomationManager } from './components/SimpleAutomationManager';
...
const App: React.FC = () => {
    ...
    const [automationRefreshTrigger, setAutomationRefreshTrigger] = useState(0); // AGENTS.MD FIX: Trigger automation refresh
    ...
    const handleSaveSettings = useCallback(async (newSettings: AppSettings) => {
        try {
            ...
            // AGENTS.MD FIX: Trigger automation manager refresh if automation settings changed
            const automationFields = ['mode', 'semiAutoIdeaFrequency', 'fullAutoDailyPostCount', 'fullAutoPublishFrequency', 'autoPublish'];
            const hasAutomationChanges = automationFields.some(field => 
                settings[field as keyof AppSettings] !== newSettings[field as keyof AppSettings]
            );
        ...
        switch (view) {
            ...
            case 'settings_license':
                ...
                />;
                    ...
                    onSaveSettings={handleSaveSettings} 
                    onShowToast={showToast}
                    automationRefreshTrigger={automationRefreshTrigger}
                />;
