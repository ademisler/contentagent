...
class ACA_Rest_Api {
    ...
    private function __construct() {
        aca_debug_log('REST API: Constructor called - Initializing REST API');
        add_action('rest_api_init', array($this, 'register_routes'));
        aca_debug_log('REST API: rest_api_init hook added');
     ...
     * @return WP_Error
     */
    private function aca_api_error($code, $message, $status_code = 400) {
    ...
    public function register_routes() {
        aca_debug_log('REST API: register_routes called - Registering all endpoints');
        
        // Check if our routes are already registered using WordPress native method
        $routes = rest_get_server()->get_routes();
        if (isset($routes['/aca/v1/ideas/generate'])) {
            aca_debug_log('REST API: ACA endpoints already registered, skipping');
            return;
        }
        ...
        register_rest_route('aca/v1', '/debug/automation-legacy', array(
            'methods' => 'GET',
        ...
        register_rest_route('aca/v1', '/generate-image', array(
            'methods' => 'POST',
        ...
        register_rest_route('aca/v1', '/automation/status', array(
            'methods' => 'GET',
            'callback' => array($this, 'get_automation_status'),
            'permission_callback' => array($this, 'check_permissions')
        ));
        
        register_rest_route('aca/v1', '/automation/test', array(
            'methods' => 'POST',
        ...
        // AGENTS.MD FIX: License refresh endpoint for automation activation issues
        register_rest_route('aca/v1', '/license/refresh', array(
            'methods' => 'POST',
        ...
        register_rest_route('aca/v1', '/trigger-scheduling', array(
            'methods' => 'POST',
        ...
        register_rest_route('aca/v1', '/cache/clear', array(
            'methods' => 'POST',
            'callback' => array($this, 'clear_cache'),
            'permission_callback' => array($this, 'check_admin_permissions'),
        ));
        
        register_rest_route('aca/v1', '/cache/status', array(
            'methods' => 'GET',
        ...
        register_rest_route('aca/v1', '/test/error-handling', array(
            'methods' => 'GET',
        ...
        register_rest_route('aca/v1', '/seo/repair-faq', array(
            'methods' => 'POST',
        ...
        register_rest_route('aca/v1', '/index/backfill', array(
            'methods' => 'POST',
        ...
        aca_debug_log('REST API: All endpoints registered successfully');
    }
    ...
    private function verify_nonce($request) {
        ...
        aca_debug_log('Nonce Verification: Received nonce: ' . ($nonce ? 'SET (' . substr($nonce, 0, 10) . '...)' : 'EMPTY'));
        
        if (!$nonce) {
            aca_debug_log('Nonce Verification: No nonce header found');
        ...
        aca_debug_log('Nonce Verification: wp_verify_nonce result: ' . ($verification_result ? 'VALID' : 'INVALID'));
        
        if (!$verification_result) {
            aca_debug_log('Nonce Verification: Invalid nonce for wp_rest action');
            return new WP_Error('invalid_nonce', __('Invalid nonce', 'ai-content-agent'), array('status' => 403));
        }
        
        aca_debug_log('Nonce Verification: Success');
        return true;
    }
    ...
    public function get_settings($request) {
        ...
        aca_debug_log('Settings Load: Retrieved settings count: ' . count($settings));
    ...
    public function save_settings($request) {
        ...
        aca_debug_log('Settings Save: Validated settings count: ' . count($validated_settings));

        // Save validated settings
        $save_result = update_option('aca_settings', $validated_settings);
        aca_debug_log('Settings Save: update_option result: ' . ($save_result ? 'SUCCESS' : 'FAILED'));
        
        // Verify settings were saved
        $saved_settings = get_option('aca_settings', array());
        aca_debug_log('Settings Save: Verification - saved settings count: ' . count($saved_settings));

        // Save Google Cloud settings separately for easy access (already validated)
        if (isset($validated_settings['googleCloudProjectId'])) {
            update_option('aca_google_cloud_project_id', $validated_settings['googleCloudProjectId']);
        }
        if (isset($validated_settings['googleCloudLocation'])) {
            update_option('aca_google_cloud_location', $validated_settings['googleCloudLocation']);
        }
        
        // Clear Google access token cache when settings change (especially API keys)
        delete_transient('aca_google_access_token');
        
        // AGENTS.MD Step 5: Enhanced automation rescheduling with force option
        // This ensures that when users change automation mode, tasks are properly scheduled
        if (class_exists('ACA_Simple_Automation')) {
            $automation = ACA_Simple_Automation::get_instance();
            
            // Check if automation-related settings changed to determine if force is needed
            $automation_settings_changed = isset($validated_settings['mode']) || 
                                         isset($validated_settings['semiAutoIdeaFrequency']) ||
                                         isset($validated_settings['fullAutoDailyPostCount']) ||
                                         isset($validated_settings['fullAutoPublishFrequency']) ||
                                         isset($validated_settings['autoPublish']);
            
            if ($automation_settings_changed) {
                // Force reschedule if automation settings changed
                $automation->force_reschedule_tasks();
                aca_debug_log('Settings Save: Forced automation task rescheduling due to automation settings change');
            } else {
                // Ensure tasks are scheduled idempotently
                $automation->ensure_scheduled_tasks();
                aca_debug_log('Settings Save: Ensured automation tasks are scheduled');
            }
        }
    ...
    private function validate_settings($settings) {
        ...
        foreach ($settings as $key => $value) {
            if (!isset($allowed_fields[$key])) {
                ...
                aca_debug_log("Settings validation: Skipping unknown field '{$key}'");
                continue;
            }
    ...
    public function generate_ideas($request) {
        ...
        try {
            ...
            if (is_array($existing_titles) && count($existing_titles) > 1000) {
                $cache_key = 'aca_existing_titles_recent_1000';
                ...
                if (!is_array($recent_subset)) {
                    ...
                    $recent_ideas = $wpdb->get_col($wpdb->prepare("SELECT title FROM {$wpdb->prefix}aca_ideas ORDER BY id DESC LIMIT %d", $limit_ideas)); // phpcs:ignore
    ...
    public function get_published_posts($request) {
        aca_debug_log('Published Posts API: Starting request');
        ...
        aca_debug_log('Published Posts API: page=' . $page . ', per_page=' . $per_page . ', offset=' . $offset);
        ...
        aca_debug_log('Published Posts API: Found ' . count($posts) . ' posts');
        aca_debug_log('Published Posts API: Query args: ' . wp_json_encode($args));
        ...
        if (!empty($posts)) {
            ...
            aca_debug_log('Published Posts API: Sample titles: ' . implode(', ', $sample_titles));
        }
        ...
        aca_debug_log('Published Posts API: Total posts available (wp_count_posts): ' . $total_posts);
        ...
        aca_debug_log('Published Posts API: Formatted ' . count($formatted_posts) . ' posts');
    ...
    public function create_draft_from_idea($idea_id, $is_auto = false) {
        ...
        try {
            ...
            try {
                ...
                aca_debug_log('FAQ generation non-blocking error: ' . $e->getMessage());
            }
    ...
    private function get_existing_titles($recent_only = false, $limit = 0) {
        ...
        if ($recent_only && $limit > 0) {
            ...
            $ideas = $wpdb->get_col($wpdb->prepare("SELECT title FROM {$wpdb->prefix}aca_ideas ORDER BY id DESC LIMIT %d", max(1, (int)floor($limit/2)))); // phpcs:ignore
            return array_merge($ideas ?: array(), $posts ?: array());
        }
        
        $cache_key = 'aca_all_existing_titles_v1';
    ...
    private function format_posts_for_api_batch($posts) {
        ...
        $meta_cache = $this->get_posts_meta_batch($post_ids, array(
            '_aca_meta_title',
            '_aca_meta_description', 
            '_aca_focus_keywords',
            '_aca_scheduled_for',
            '_aca_ai_generated',
            '_aca_generation_date'
        ));
        ...
        foreach ($posts as $post) {
            try {
                ...
                $formatted_posts[] = array(
                    ...
                    'metaTitle' => $meta_cache[$post_id]['_aca_meta_title'] ?? '',
                    'metaDescription' => $meta_cache[$post_id]['_aca_meta_description'] ?? '',
                    'focusKeywords' => $meta_cache[$post_id]['_aca_focus_keywords'] ?? '',
                    ...
                    'scheduledFor' => $post->post_status === 'future' ? $post->post_date : ($meta_cache[$post_id]['_aca_scheduled_for'] ?? ''),
                    'scheduledForIso' => $post->post_status === 'future' ? wp_date('c', strtotime($post->post_date)) : ($meta_cache[$post_id]['_aca_scheduled_for'] ? wp_date('c', strtotime($meta_cache[$post_id]['_aca_scheduled_for'])) : ''),
                    'aiGenerated' => !empty($meta_cache[$post_id]['_aca_ai_generated']),
                    'generationDate' => $meta_cache[$post_id]['_aca_generation_date'] ?? ''
                );
                
            } catch (Exception $e) {
                aca_debug_log('Format Post Batch Error for post ' . $post->ID . ': ' . esc_html($e->getMessage()));
    ...
    private function format_published_posts_for_api($posts) {
        ...
        foreach ($posts as $post) {
            try {
                ...
                $focus_keywords = $meta_cache[$post_id]['_aca_focus_keywords'] ?? '';
                ...
                $formatted_posts[] = array(
                    ...
                    'content' => $post->post_content ?: '',
                    'excerpt' => $excerpt ?: '',
                    'metaTitle' => $meta_cache[$post_id]['_aca_meta_title'] ?? $post->post_title,
                    'metaDescription' => $meta_cache[$post_id]['_aca_meta_description'] ?? $excerpt,
                    'focusKeywords' => $focus_keywords,
                    'categories' => $categories_cache[$post_id] ?? array(),
                    'tags' => $tags_cache[$post_id] ?? array(),
                    'featuredImage' => $thumbnail_cache[$post_id] ?? '',
                ...
                aca_debug_log('Format Published Post Error for post ' . $post->ID . ': ' . esc_html($e->getMessage()));
                // Continue with other posts, don't fail entire batch
                continue;
            }
        }
    ...
    private function get_posts_meta_batch($post_ids, $meta_keys) {
        ...
        $cache_key = 'aca_meta_batch_' . md5(wp_json_encode(array('p' => array_map('intval', $post_ids), 'k' => array_map('strval', $meta_keys))));
    ...
    private function format_post_for_api($post) {
        try {
            ...
            try {
                ...
                $scheduled_for = $post->post_status === 'future' ? $post->post_date : (get_post_meta($post->ID, '_aca_scheduled_for', true) ?: '');
                        ...
                        if ($orig_w > 0 && $orig_h > 0) {
                            ...
                            if (!is_wp_error($crop_result)) {
                                ...
                                if (!is_wp_error($save_result)) {
                                    ...
                                    aca_debug_log('Image crop save failed: ' . $save_result->get_error_message());
                                }
                            } else {
                                aca_debug_log('Image crop failed: ' . $crop_result->get_error_message());
                            }
                        }
                    }
                } else {
                    aca_debug_log('Image editor init failed: ' . $editor->get_error_message());
                }
            }
        }
    ...
    private function build_image_search_query($title, $content_html, $settings) {
        ...
        if (!empty($settings['geminiApiKey'])) {
            try {
                ...
                aca_debug_log('Image query keywords via AI failed: ' . $e->getMessage());
  ...
  \"excerpt\": \"Brief excerpt for the post (150 characters)\"
    ...
    private function fetch_stock_photo($query, $provider, $api_key) {
        ...
        switch ($provider) {
            ...
            case 'pixabay':
                ...
                aca_debug_log('Security Warning: Pixabay API requires key in URL - consider alternative providers for sensitive environments');
    ...
    public function debug_automation($request) {
        ...
        $debug_info = array(
            ...
            'unified_automation' => array(
                'class_loaded' => class_exists('ACA_Simple_Automation'),
                'instance_active' => method_exists('ACA_Simple_Automation', 'get_instance'),
                'hooks_registered' => has_action('aca_simple_idea_generation'),
                'system_status' => 'Unified & Reliable'
            ),
            ...
            // Enhanced debugging info for unified system
            'wordpress_cron' => array(
                'enabled' => !defined('DISABLE_WP_CRON') || !DISABLE_WP_CRON,
                'next_scheduled' => wp_next_scheduled('aca_simple_idea_generation'),
                'cron_disabled' => defined('DISABLE_WP_CRON') && DISABLE_WP_CRON
            ),

            'unified_system' => array(
                'class_loaded' => class_exists('ACA_Simple_Automation'),
                'instance_active' => method_exists('ACA_Simple_Automation', 'get_instance'),
                'hooks_registered' => has_action('aca_simple_idea_generation'),
                'system_status' => 'Unified & Reliable'
    ...
    public function debug_trigger_semi_auto($request) {
        try {
            ...
            $this->add_activity_log('debug_cron', __('Semi-automatic task manually triggered via Simple Automation', 'ai-content-agent'), 'Settings');
            update_option('aca_last_cron_run', current_time('mysql') . ' (Semi-Auto Manual - Simple)');
            
            return rest_ensure_response(array(
                'success' => true,
                'message' => __('Semi-automatic task executed successfully', 'ai-content-agent'),
                'result' => $result,
                'timestamp' => current_time('mysql')
            ));
        } catch (Exception $e) {
    ...
    public function debug_trigger_full_auto($request) {
        try {
            ...
            $this->add_activity_log('debug_cron', __('Full-automatic task manually triggered via Simple Automation', 'ai-content-agent'), 'Settings');
            update_option('aca_last_cron_run', current_time('mysql') . ' (Full-Auto Manual - Simple)');
            
            return rest_ensure_response(array(
                'success' => true,
                'message' => __('Full-automatic task executed successfully', 'ai-content-agent'),
                'result' => $result,
                'timestamp' => current_time('mysql')
            ));
        } catch (Exception $e) {
    ...
    public function get_seo_plugins($request) {
        try {
            ...
            aca_debug_log('Error in get_seo_plugins: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    ...
    private function detect_seo_plugin() {
        ...
        if (!function_exists('is_plugin_active')) {
            ...
            if (file_exists($plugin_file)) {
                ...
                aca_debug_log('Warning: plugin.php file not found at: ' . $plugin_file);
        ...
        aca_debug_log('Active plugins count: ' . count($active_plugins));
        aca_debug_log('Network active plugins count: ' . count($active_network_plugins));
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            if ($rankmath_detected) {
                aca_debug_log(' RankMath in active_plugins: ' . ($rankmath_in_active ? 'yes' : 'no'));
            }
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            if ($yoast_detected) {
                aca_debug_log(' Yoast in active_plugins: ' . ($yoast_in_active ? 'yes' : 'no'));
            }
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            if ($aioseo_detected) {
                aca_debug_log(' AIOSEO in active_plugins: ' . ($aioseo_in_active ? 'yes' : 'no'));
            }
        }
        ...
        aca_debug_log('is_plugin_active function exists: ' . (function_exists('is_plugin_active') ? 'yes' : 'no'));
        aca_debug_log('ABSPATH: ' . ABSPATH);
        ...
        aca_debug_log('RankMath file exists: ' . (file_exists($rankmath_file) ? 'yes' : 'no'));
        aca_debug_log('Yoast file exists: ' . (file_exists($yoast_file) ? 'yes' : 'no'));
        aca_debug_log('AIOSEO file exists: ' . (file_exists($aioseo_file) ? 'yes' : 'no'));
        
        // If no plugins detected, check why
        if (empty($detected_plugins)) {
            aca_debug_log('WARNING: No SEO plugins detected!');
            aca_debug_log('Checking constants:');
            aca_debug_log('- RANK_MATH_VERSION defined: ' . (defined('RANK_MATH_VERSION') ? 'yes' : 'no'));
            aca_debug_log('- WPSEO_VERSION defined: ' . (defined('WPSEO_VERSION') ? 'yes' : 'no'));
            aca_debug_log('- AIOSEO_VERSION defined: ' . (defined('AIOSEO_VERSION') ? 'yes' : 'no'));
        }
    ...
    private function send_to_rankmath($post_id, $meta_title, $meta_description, $focus_keywords) {
        try {
            ...
            return $this->aca_api_error('rankmath_error', sprintf(
                /* translators: %s: error message */
    ...
    private function send_to_yoast($post_id, $meta_title, $meta_description, $focus_keywords) {
        ...
        if (!empty($meta_description)) {
            ...
            return $this->aca_api_error('yoast_error', sprintf(
                /* translators: %s: error message */
    ...
    private function send_to_aioseo($post_id, $meta_title, $meta_description, $focus_keywords) {
        try {
            ...
            return $this->aca_api_error('aioseo_error', sprintf(
                /* translators: %s: error message */
    ...
    public function verify_license_key($request) {
        ...
        if (empty($license_key)) {
            return new WP_Error(
                'missing_license_key', 
                __('License key is required', 'ai-content-agent'), 
                array('status' => 400)
            );
        }
        ...
        try {
            ...
            if ($verification_result['success']) {
                ...
                if (!empty($stored_site_hash) && $stored_site_hash !== $current_site_hash) {
                    ...
                    return $this->aca_api_error('license_site_bound', __('This license is already active on another website. Each license can only be used on one site at a time. Please deactivate it from the other site first.', 'ai-content-agent'), 409);
                }
                ...
                $verification_result['message'] = __('License verification failed. Please check your license key and try again.', 'ai-content-agent');
            }
            ...
            // Return consistent WP_Error response
            return $this->aca_api_error('license_verification_failed', sprintf(
                /* translators: %s: error message */
                __('License verification failed: %s', 'ai-content-agent'),
                $e->getMessage()
            ), 500);
        }
    }
    ...
    public function deactivate_license($request) {
        ...
        try {
            ...
            return rest_ensure_response(array(
                'success' => true,
                'message' => __('License deactivated successfully. You can now use this license on another site.', 'ai-content-agent')
            ));
            ...
            // Return consistent WP_Error response
            return $this->aca_api_error('license_deactivation_failed', sprintf(
                /* translators: %s: error message */
                __('License deactivation failed: %s', 'ai-content-agent'),
                $e->getMessage()
            ), 500);
        }
    }
    ...
    private function call_gumroad_api($product_id, $license_key) {
        ...
        if (is_wp_error($response)) {
            ...
            return array(
                ...
                'message' => sprintf(
                    /* translators: %s: error message */
                    __('Gumroad API request failed: %s', 'ai-content-agent'),
                    $response->get_error_message()
                ),
                'purchase_data' => null,
        ...
        if ($response_code !== 200) {
            ...
            return array(
                ...
                'message' => sprintf(
                    ...
                    __('Gumroad API returned error code: %s', 'ai-content-agent'),
                    $response_code
                ),
                'purchase_data' => null,
                'verified_at' => current_time('mysql'),
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            return array(
                'success' => false,
                'message' => __('Invalid JSON response from Gumroad API', 'ai-content-agent'),
                'purchase_data' => null,
                'verified_at' => current_time('mysql'),
        ...
        if (!isset($data['success'])) {
            ...
            return array(
                'success' => false,
                'message' => __('Invalid Gumroad API response format', 'ai-content-agent'),
                'purchase_data' => null,
                'verified_at' => current_time('mysql'),
        ...
        return array(
            'success' => $is_valid,
            'message' => $is_valid ? __('License verified successfully', 'ai-content-agent') : __('License verification failed', 'ai-content-agent'),
    ...
    public function generate_image($request) {
        try {
            ...
            if (empty($prompt)) {
                return $this->aca_api_error('missing_prompt', 'Prompt parameter is required', 400);
            }
            ...
            aca_debug_log('Image Generation Error: ' . $e->getMessage());
            return $this->aca_api_error('image_generation_error', sprintf(
                /* translators: %s: error message */
    ...
    public function get_automation_status() {
        try {
            ...
            aca_debug_log('Automation Status Error: ' . $e->getMessage());
            return $this->aca_api_error('automation_error', sprintf(
                /* translators: %s: error message */
    ...
    public function test_automation() {
        try {
            ...
            aca_debug_log('Automation Test Error: ' . $e->getMessage());
            return $this->aca_api_error('automation_test_error', sprintf(
                /* translators: %s: error message */
    ...
    public function debug_automation_status($request) {
        try {
            ...
            $health_checks = array(
                ...
                'simple_automation_loaded' => class_exists('ACA_Simple_Automation'),
                'unified_system_active' => class_exists('ACA_Simple_Automation') && method_exists('ACA_Simple_Automation', 'get_instance'),
                'unified_system_status' => __('Unified & Reliable', 'ai-content-agent'),
                'scheduled_events' => array(
                    'idea_generation' => wp_next_scheduled('aca_simple_idea_generation'),
                    'draft_creation' => wp_next_scheduled('aca_simple_draft_creation'),
                    'post_publishing' => wp_next_scheduled('aca_simple_post_publishing'),
                    'maintenance' => wp_next_scheduled('aca_simple_maintenance')
                ),
                'next_run_times' => array(
                    'idea_generation' => wp_next_scheduled('aca_simple_idea_generation') ? gmdate('Y-m-d H:i:s', wp_next_scheduled('aca_simple_idea_generation')) : __('Not scheduled', 'ai-content-agent'),
                    'draft_creation' => wp_next_scheduled('aca_simple_draft_creation') ? gmdate('Y-m-d H:i:s', wp_next_scheduled('aca_simple_draft_creation')) : __('Not scheduled', 'ai-content-agent'),
                    'post_publishing' => wp_next_scheduled('aca_simple_post_publishing') ? gmdate('Y-m-d H:i:s', wp_next_scheduled('aca_simple_post_publishing')) : __('Not scheduled', 'ai-content-agent'),
                    'maintenance' => wp_next_scheduled('aca_simple_maintenance') ? gmdate('Y-m-d H:i:s', wp_next_scheduled('aca_simple_maintenance')) : __('Not scheduled', 'ai-content-agent')
                )
            );
            ...
            return rest_ensure_response(array(
                ...
                'system_info' => array(
                    ...
                    'plugin_version' => get_option('aca_plugin_version', __('Unknown', 'ai-content-agent'))
                ),
                'wp_cron_disabled' => defined('DISABLE_WP_CRON') && DISABLE_WP_CRON,
                'memory_limit' => WP_MEMORY_LIMIT,
                'cron_jobs' => array(
                    'aca_simple_idea_generation' => wp_next_scheduled('aca_simple_idea_generation'),
                    'aca_simple_draft_creation' => wp_next_scheduled('aca_simple_draft_creation'),
                    'aca_simple_post_publishing' => wp_next_scheduled('aca_simple_post_publishing'),
                    'aca_simple_maintenance' => wp_next_scheduled('aca_simple_maintenance')
                ),
            ...
            return $this->aca_api_error('debug_failed', sprintf(
                /* translators: %s: error message */
    ...
    /**
     * PHASE 1 CONSOLIDATION: Queue Manager deprecated - use ACA_Simple_Automation
     */
    public function schedule_queue_task($request) {
        aca_debug_log('PHASE 1: Queue Manager deprecated - use ACA_Simple_Automation unified system');
        return $this->aca_api_error('deprecated_system', __('Queue Manager deprecated. Use ACA_Simple_Automation unified system.', 'ai-content-agent'), 410);
    }
    ...
    public function get_automation_system_status() {
        try {
            global $aca_content_agent;
            if ($aca_content_agent && method_exists($aca_content_agent, 'get_automation_system_status')) {
                $status = $aca_content_agent->get_automation_system_status();
            } else {
            ...
            $status['diagnostics'] = array(
                ...
                'wp_cron_disabled' => defined('DISABLE_WP_CRON') && DISABLE_WP_CRON,
                'unified_automation_hooks_registered' => has_action('aca_simple_idea_generation'),
                'unified_system_status' => __('Unified & Reliable', 'ai-content-agent')
            );
            ...
            aca_debug_log('Automation System Status Error: ' . $e->getMessage());
            return $this->aca_api_error('automation_system_error', 'Failed to get automation system status: ' . $e->getMessage(), 500);
        }
    }
    ...
    public function get_queue_actions_deprecated($request) {
        return $this->aca_api_error('deprecated', __('This endpoint is deprecated. Use /automation/status instead.', 'ai-content-agent'), 410);
    }
    ...
    public function cancel_all_queue_tasks_deprecated($request) {
        return $this->aca_api_error('deprecated', __('This endpoint is deprecated. Unified automation system does not use queues.', 'ai-content-agent'), 410);
    }
    ...
    public function trigger_scheduling($request) {
        try {
            aca_debug_log('REST API: Manual scheduling trigger requested');
            
            // AGENTS.MD FIX: Force license verification cache refresh for automation activation
            $pro_status_before = is_aca_pro_active(false);
            $pro_status_after = is_aca_pro_active(true);
            ...
            aca_debug_log('REST API: Manual scheduling trigger completed');
            ...
            return new WP_REST_Response(array(
                ...
                'status' => $status,
                'next_runs' => array(
                    'idea_generation' => wp_next_scheduled(ACA_Simple_Automation::HOOK_IDEA_GENERATION),
                    'draft_creation' => wp_next_scheduled(ACA_Simple_Automation::HOOK_DRAFT_CREATION),
                    'post_publishing' => wp_next_scheduled(ACA_Simple_Automation::HOOK_POST_PUBLISHING),
                    'maintenance' => wp_next_scheduled(ACA_Simple_Automation::HOOK_MAINTENANCE)
                )
            ), 200);
            
        } catch (Exception $e) {
            aca_debug_log('REST API: Error in manual scheduling trigger - ' . $e->getMessage());
    ...
    /**
     * AGENTS.MD FIX: Refresh license status endpoint for automation activation issues
     * Forces cache refresh and re-triggers automation scheduling
     */
    public function refresh_license_status($request) {
        try {
            aca_debug_log('REST API: License refresh requested');
            
            // Force license verification cache refresh
            $pro_status_before = is_aca_pro_active(false);
            $pro_status_after = is_aca_pro_active(true);
            
            aca_debug_log(sprintf(
                'REST API: License refresh - Before: %s, After: %s',
                $pro_status_before ? 'ACTIVE' : 'INACTIVE',
                $pro_status_after ? 'ACTIVE' : 'INACTIVE'
            ));
            
            // If pro status changed, trigger automation rescheduling
            if (!$pro_status_before && $pro_status_after) {
                $automation = ACA_Simple_Automation::get_instance();
                if ($automation) {
                    $automation->ensure_scheduled_tasks();
                    aca_debug_log('REST API: License became active - triggered automation rescheduling');
                }
            }
            
            return new WP_REST_Response(array(
                'success' => true,
                'message' => __('License status refreshed successfully', 'ai-content-agent'),
                'pro_status_before' => $pro_status_before,
                'pro_status_after' => $pro_status_after,
                'license_debug' => array(
            ...
            aca_debug_log('REST API: Error in license refresh - ' . $e->getMessage());
            return new WP_Error('license_refresh_failed', sprintf(
                /* translators: %s: error message */
                __('Failed to refresh license status: %s', 'ai-content-agent'),
                $e->getMessage()
            ), array('status' => 500));
        }
    }
    ...
    public function clear_cache($request) {
        try {
            ...
            $result = ACA_Cache_Manager::clear_all_caches('REST API - Manual');
            
            return new WP_REST_Response($result, 200);
        } catch (Throwable $e) {
            aca_debug_log('REST API: Cache clear failed - ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    ...
    public function get_cache_status($request) {
        try {
            ...
            $status = ACA_Cache_Manager::get_cache_status();
            
            return new WP_REST_Response($status, 200);
        } catch (Throwable $e) {
            aca_debug_log('REST API: Failed to get cache status - ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
     ...
     */
    public function get_automation_debug_info($request) {
        try {
            // Get automation instance
            $automation = ACA_Simple_Automation::get_instance();
            $status = $automation->get_status();
            ...
            aca_debug_log('REST API: Debug info failed - ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    ...
    private function append_faqs_jsonld_to_content($post_id, $faqs) {
        ...
        if ($post && !is_wp_error($post)) {
            if (strpos($post->post_content, '<!-- ACA_FAQ_JSONLD -->') !== false) {
        ...
        update_post_meta($post_id, '_aca_faq_jsonld', wp_json_encode($jsonld));
        ...
        if (!empty($settings['faqDisplayInContent'])) {
            ...
            if ($post && !is_wp_error($post)) {
                ...
                if (strpos($post->post_content, '<!-- ACA_FAQ_VISIBLE -->') === false) {
                    $html = "<!-- ACA_FAQ_VISIBLE -->\n<div class=\"aca-faq\"><h2>SSS</h2><div class=\"aca-faq-items\">";
                    foreach ($faqs as $qa) {
                        $q = esc_html($qa['question']);
                        $a = wp_kses_post($qa['answer']);
                        $html .= "<details class=\"aca-faq-item\"><summary>{$q}</summary><div class=\"aca-faq-answer\">{$a}</div></details>";
                    }
                    $html .= "</div></div>\n<!-- /ACA_FAQ_VISIBLE -->";
    ...
    private function cleanup_rankmath_schema_if_invalid($post_id) {
        ...
        if (is_string($value) && !empty($value)) {
            ...
            aca_debug_log('Rank Math schema cleanup: removed string meta for post ' . $post_id);
        }
    }
    ...
    private function get_internal_link_candidates_for_idea($idea_title, $site_locale, $top_k = 5) {
        ...
        if (empty($rows)) {
            ...
            $rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
                $wpdb->prepare(
                    'SELECT post_id, title, url, summary_1l, keywords FROM ' . $wpdb->prefix . 'aca_post_index WHERE (lang LIKE %s OR lang IS NULL) ORDER BY updated_at DESC LIMIT %d',
                    $like_lang,
                    300
                ),
                ARRAY_A
    ...
    public function index_backfill($request) {
        ...
        $lock_key = 'aca_index_backfill_lock';
        ...
        try {
            if ($reset) {
                delete_option('aca_index_backfill_last_id');
                delete_option('aca_index_backfill_done');
            }
            if (get_option('aca_index_backfill_done')) {
                return rest_ensure_response(array('done' => true, 'processed' => 0, 'last_id' => (int)get_option('aca_index_backfill_last_id', 0)));
            }

            $last_id = (int)get_option('aca_index_backfill_last_id', 0);
            ...
            if (empty($rows)) {
                update_option('aca_index_backfill_done', 1);
            ...
            update_option('aca_index_backfill_last_id', $last_id);
