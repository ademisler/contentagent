...
const makeApiCallWithRetry = async (
    ...
    try {
        ...
        if (isOverloadError && retryCount < modelConfig.maxRetries) {
            ...
            console.log(`ACA: Retrying ${modelName} in ${delay}ms... (attempt ${retryCount + 2}/${modelConfig.maxRetries + 1})`);
...
const generateImage = async (prompt: string, style: AiImageStyle = 'digital_art'): Promise<string> => {
    try {
        ...
        const response = await fetch(`${window.acaData.apiUrl}generate-image`, {
            method: 'POST',
            headers: {
