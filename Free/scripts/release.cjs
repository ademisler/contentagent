...
const PLUGIN_DIR = '/workspace/Free/plugin';
const OUT_DIR = '/workspace/Free/release';

function run(cmd, cwd = '/workspace/Free') {
  console.log(`$ ${cmd}`);
...
function main() {
  ...
  build();
  if (changelogEntries) {
    run(`node /workspace/Free/scripts/generate-changelog.cjs ${version} ${new Date().toISOString().slice(0,10)} "${changelogEntries}"`);
  }
  zipMinimal(version);
}
