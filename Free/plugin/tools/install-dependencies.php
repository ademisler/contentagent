...
class AICOAGAC_Dependencies_Installer {
    
    public function __construct() {
        add_action('wp_ajax_aicoagac_install_dependencies', array($this, 'install_dependencies'));
    }
    ...
    public static function are_dependencies_installed() {
        ...
        return file_exists(AICOAGAC_PLUGIN_PATH . 'vendor/autoload.php');
    }
    ...
    public static function get_dependency_status() {
        $vendor_exists = file_exists(AICOAGAC_PLUGIN_PATH . 'vendor/autoload.php');
        $composer_json_exists = file_exists(AICOAGAC_PLUGIN_PATH . 'composer.json');
    ...
    public function install_dependencies() {
        ...
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'aicoagac_install_dependencies')) {
    ...
    private function run_composer_install() {
        ...
        $plugin_dir = AICOAGAC_PLUGIN_PATH;
        $output = array();
        $return_code = 0;
    ...
    private function get_manual_instructions() {
        return array(
            ...
            'steps' => array(
                ...
                sprintf( __( '2. Navigate to: %s', 'ai-content-agent' ), AICOAGAC_PLUGIN_PATH ),
    ...
    public static function display_dependency_status() {
        ...
        if ($status['can_auto_install']) {
            echo '<p><button id="aicoagac-install-deps" class="button button-primary">' . esc_html__( 'Install Dependencies Automatically', 'ai-content-agent' ) . '</button></p>';
        } else {
            ...
            echo '<li>' . sprintf( esc_html__( 'Navigate to: %s', 'ai-content-agent' ), '<code>' . esc_html(AICOAGAC_PLUGIN_PATH) . '</code>' ) . '</li>';
        ...
        if ($status['can_auto_install']) {
            ...
            jQuery(document).ready(function($) {
                $('#aicoagac-install-deps').click(function() {
                    var button = $(this);
                    button.prop('disabled', true).text('<?php echo esc_js( __( 'Installing...', 'ai-content-agent' ) ); ?>');
                    
                    $.post(ajaxurl, {
                        action: 'aicoagac_install_dependencies',
                        nonce: '<?php echo esc_js(wp_create_nonce('aicoagac_install_dependencies')); ?>'
                    }, function(response) {
                        if (response.success) {
...
new AICOAGAC_Dependencies_Installer();